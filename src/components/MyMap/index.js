...
import { BuildingsLayer2D, BuildingsLayerColored, clusterCountLayer, clusterLayer, unclusteredPointLayer } from "./Layers";
import { getBuildings, pruneUnique, numFormatter } from "../../shared/utilities";
...
const MyMap = React.forwardRef((props, ref) => {
  ...
  const drawColorBuildings = (e) => {
    ...
    if(map && map.getZoom()>16){
      if (map.areTilesLoaded()) 
      {
        ...
        var buildings = getBuildings(map, 16.01);
  ...
  const updateSuburbs = ()=>{
    ...
    fetch('http://www.homiee.a2hosted.com/api/v1/index.php/property/listmap')
    .then(res => res.json())
    .then(json => {
      window.mapdata = json;
      pruneUnique(window.mapdata.features);
